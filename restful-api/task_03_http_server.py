...
class SimpleHTTPServer(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/data":
            ...
            self.send_response_header_write(200, json.dumps(simple_json_dataset),\
                                            "application/json")
        elif self.path == "/info":
            info_json_dataset = \
            {
            ...
            self.send_response_header_write(200, "Hello, this is a simple API!")
        else:
            self.send_response_header_write(404, "Endpoint not found")
            # 404 Not found (error)

    def send_response_header_write(self, status=200, write="", datatype="text/html"):
